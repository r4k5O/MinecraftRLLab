...
class LogMuxTests(unittest.TestCase):
    ...
    def test_ninja_style_progress_lines_are_nearly_all_visible(self):
        ...
        visible = [
            ...
            if sampler.should_emit(
                line := f"[{i}/1651] Building CXX object src/file{i}.cpp.obj"
            )
        ]
    ...
    def test_scons_compiler_commands_are_nearly_all_visible(self):
        ...
        visible = [line for line in lines if sampler.should_emit(line)]
        self.assertGreaterEqual(len(visible), 450)

        windows = [f'C:\\LLVM\\bin\\cl.exe /c module{i}.c /Fomodule{i}.obj' for i in range(100)]
        visible_windows = [line for line in windows if sampler.should_emit(line)]
        self.assertGreaterEqual(len(visible_windows), 90)
    ...
    def test_run_logged_preserves_full_log_and_dense_progress_console(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            cmd = [
                ...
                "-S",
                "-c",
                "for i in range(1651): print(f'[{i+1}/1651] Building CXX object x{i}.cpp')",
            ]
            ...
            visible_compile_lines = [
                line
                for line in out.getvalue().splitlines()
                if "Building CXX object" in line
            ]
            self.assertEqual(len(full_lines), 1651)
